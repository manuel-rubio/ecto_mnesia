defmodule EctoMnesia.Storage.Migrator do
  ...
  def lock_for_migrations(_adapter_meta, _opts, function) do
    function.()
  end

  @doc false
  # Tables
  def execute(adapter_meta, {:create, %Ecto.Migration.Table{name: table, engine: type}, instructions}, _opts) do
    ensure_pk_table!(adapter_meta)
  ...
  def execute(adapter_meta, {:create_if_not_exists, %Ecto.Migration.Table{name: table, engine: type}, instructions}, _opts) do
    ensure_pk_table!(adapter_meta)
  ...
  def execute(adapter_meta, {:alter, %Ecto.Migration.Table{name: table}, instructions}, _opts) do
    ensure_pk_table!(adapter_meta)
  ...
  def execute(adapter_meta, {:rename, %Ecto.Migration.Table{name: table}, old_field, new_field}, _opts) do
    ensure_pk_table!(adapter_meta)
    table = if String.valid?(table), do: String.to_atom(table), else: table
  ...
  def execute(_adapter_meta, {:drop, %Ecto.Migration.Table{name: table}}, _opts) do
    table = if String.valid?(table), do: String.to_atom(table), else: table
  ...
  def execute(_adapter_meta, {:drop_if_exists, %Ecto.Migration.Table{name: table}}, _opts) do
    table = if String.valid?(table), do: String.to_atom(table), else: table
  ...
  # Indexes
  def execute(_adapter_meta, {:create, %Ecto.Migration.Index{table: table, columns: columns}}, _opts) do
    table = if String.valid?(table), do: String.to_atom(table), else: table
  ...
  def execute(_adapter_meta, {:create_if_not_exists, %Ecto.Migration.Index{table: table, columns: columns}}, _opts) do
    table = if String.valid?(table), do: String.to_atom(table), else: table
  ...
  def execute(_adapter_meta, {:drop, %Ecto.Migration.Index{table: table, columns: columns}}, _opts) do
    table = if String.valid?(table), do: String.to_atom(table), else: table
  ...
  def execute(_adapter_meta, {:drop_if_exists, %Ecto.Migration.Index{table: table, columns: columns}}, _opts) do
    table = if String.valid?(table), do: String.to_atom(table), else: table
  ...
  # Helpers
  defp do_create_table(adapter_meta, table, type, attributes) do
    config = conf(adapter_meta)
  ...
  defp ensure_pk_table!(adapter_meta) do
    res =
      try do
    ...
    case res do
      :no_exists ->
        do_create_table(adapter_meta, @pk_table_name, :set, [:thing, :id])
  ...
  defp conf(%{repo: repo}), do: EctoMnesia.Storage.conf(repo.config)
end
